...
NST table pages have a dominant <table> with predictable headers. We use
pandas.read_html, then normalize column names via a per-shape rename dict.
"""
...
def _normalize(df: pd.DataFrame, renames: dict[str, str]) -> pd.DataFrame:
    # NST pages sometimes have a leading unnamed index column.
    df = df.copy()
    ...
    # Keep only known columns + 'team'/'name' passthroughs we don't know about.
    return df
...
def _read_first_table(html: bytes | str) -> pd.DataFrame:
    ...
    # NST team/skater pages use the largest table; pick by row count.
...
def parse_toi_minutes(val) -> float:
    """NST TOI is sometimes formatted 'mm:ss' on game-log views; totals are minutes."""
    if pd.isna(val):
        return 0.0
