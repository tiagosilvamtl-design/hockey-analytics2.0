"""Branded EN+FR docx renderer for tag-cohort split-study findings.

Given any archetype tag in the corpus, renders a standalone analytical
report:
  - Cohort identification (which players, with confidence + source quote)
  - Reg-season vs playoff iso-net delta per player
  - Aggregate mean + 80% empirical CI
  - Honest read: does the data support the archetype-level claim?

Usage:
    PYTHONIOENCODING=utf-8 .venv/Scripts/python tools/render_cohort_study.py warrior
    PYTHONIOENCODING=utf-8 .venv/Scripts/python tools/render_cohort_study.py playmaker --min-confidence 0.7
"""
...
from lemieux.core import find_players_by_tag, tag_split_study

DB = REPO / "legacy" / "data" / "store.sqlite"
OUT_DIR = REPO / "examples" / "cohort_studies"
OUT_DIR.mkdir(parents=True, exist_ok=True)


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("tag", help="Archetype tag to study (e.g. warrior, playmaker, sniper)")
    ap.add_argument("--min-confidence", type=float, default=0.6,
                    help="Min tag confidence for cohort inclusion")
    ap.add_argument("--min-reg-toi", type=float, default=200.0)
    ap.add_argument("--min-playoff-toi", type=float, default=50.0)
    args = ap.parse_args()

    con = sqlite3.connect(DB, timeout=60)

    cohort = find_players_by_tag(con, args.tag, min_confidence=args.min_confidence)
    if not cohort:
        print(f"No players tagged '{args.tag}' at confidence ≥ {args.min_confidence}.")
        return

    result = tag_split_study(
        con, args.tag,
        min_tag_confidence=args.min_confidence,
        min_reg_toi=args.min_reg_toi,
        min_playoff_toi=args.min_playoff_toi,
    )
    ...
    payload = {
        ...
        "cohort": [
            {"name": p.name, "position": p.position, "confidence": p.confidence,
             "source_quote": p.source_quote[:200], "source_url": p.source_url}
            for p in cohort
        ],
        "study": {
            "n_players_in_split": result.n_players,
            "mean_delta_iso_net": result.mean_delta_iso_net,
            "median_delta_iso_net": result.median_delta_iso_net,
            "ci80_low": result.ci80_low,
            "ci80_high": result.ci80_high,
            "ci80_excludes_zero": (result.ci80_low * result.ci80_high) > 0,
            "per_player": result.per_player_deltas,
        },
    }
    json_path = OUT_DIR / f"{args.tag}_split_study.numbers.json"
    json_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    print(f"Wrote {json_path}")

    # Print summary to console
    print(f"\nCohort tagged '{args.tag}' (min_confidence ≥ {args.min_confidence}):")
    print(f"  Total tagged: {len(cohort)}")
    print(f"  In split-study (with both reg + playoff TOI): {result.n_players}")
    if result.n_players >= 3:
        print(f"  Mean Δ: {result.mean_delta_iso_net:+.3f}")
        print(f"  Median Δ: {result.median_delta_iso_net:+.3f}")
        print(f"  80% empirical CI: [{result.ci80_low:+.3f}, {result.ci80_high:+.3f}]")
        sign = "yes" if (result.ci80_low * result.ci80_high) > 0 else "no — straddles zero"
        print(f"  CI sign-consistent: {sign}")
        print(f"\nPer-player breakdown:")
        for r in sorted(result.per_player_deltas, key=lambda x: -x["delta"]):
            print(f"    {r['name']:25s} reg={r['reg_iso_net']:+.3f} (toi {r['reg_toi']:.0f})  "
                  f"playoff={r['playoff_iso_net']:+.3f} (toi {r['playoff_toi']:.0f})  "
                  f"Δ={r['delta']:+.3f}")

    # Render docx via Node script (kept minimal — heavy renderers go through node)
    render_js = OUT_DIR / "build_cohort_study_post.js"
    if not render_js.exists():
        # Write the renderer once
        render_js.write_text(_render_template(), encoding="utf-8")
    # Invoke via node
    print(f"\nRendering EN+FR docx via {render_js.name} ...")
    try:
        r = subprocess.run(
            ["node", str(render_js), str(json_path)],
        ...
        print("  (node not on PATH — skipping docx render. JSON is persisted.)")
    except Exception as e:
        print(f"  Renderer failed: {e}")


def _render_template() -> str:
    """The Node renderer source. Minimal docx with the cohort + study results."""
    return r"""// Branded EN+FR docx for a tag-cohort split study.
